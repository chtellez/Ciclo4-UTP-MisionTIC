...
exports.update = function (req, res) {
  ...
  Empleado.findByIdAndUpdate(req.params.id, { $set: empleado }, function (err) {
    if (err) {
      console.err(err),
        (response.exito = false),
        (response.msg = "Error al cambiar datos de empleados");
      res.json(response);
      return;
    }
    response.exito = true;
...
exports.remove = function (req, res) {
  Empleado.findByIdAndRemove({ _id: req.params.id }, function (err) {
    if (err) {
      console.error(err),
        (response.exito = false),
        (response.msg = "Error al eliminar el empleados"),
        res.json(response);
      return;
    }
    (response.exito = true), (response.msg = "El empleado ha sido eliminado");
  });
};
